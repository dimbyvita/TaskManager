...
export const CalendarPM = (): ReactElement => {
  const { nav, days, monthDisplay, setNav,handleTodayClick } = useMonthCalendar();

  return (
    <div className='absolute right-0 mt-2 w-72 h-80 bg-white text-gray-800 hover:text-teal-700 shadow-xl rounded-xl overflow-hidden z-50'>
      <div className='bg-slate-200 flex flex-col text-sm gap-3 p-2 rounded-md h-full'>
            ...
            <tbody>
              ...
              {Array.from({ length: Math.ceil(days.length / 7) }, (_, rowIndex) => (
                ...
                {days.slice(rowIndex * 7, rowIndex * 7 + 7).map((day: Day, index: number) => (
                  <td
                    key={index}
                    className={`relative border border-gray-300 bg-lime-400/20 hover:bg-gradient-to-r from-blue-200 to-violet-200 
                      cursor-pointer
